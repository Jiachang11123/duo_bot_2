...
# ⚠️⚠️⚠️ 請將這裡換成你自己抓到的那串 SKILL_COMPLETION ID ⚠️⚠️⚠️
MAGIC_ID = "SKILL_COMPLETION_BALANCED-xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx-1-GEMS"

# ==========================================
# ⚡ 黃金參數 (Golden Settings)
# ==========================================
# 這是經過測試最穩定的極限值，請勿隨意調大
DEFAULT_THREADS = 7    # 線程數
DEFAULT_BATCH = 45     # 單次批量
DEFAULT_DELAY = 0.15   # 請求間隔 (秒)
NOTIFY_INTERVAL = 1800 # 30分鐘發一次通知
...
class DuoGemNuclear:
    def __init__(self, token, reward_id):
        ...
        self.start_time = 0
        self.initial_gems = 0
        self.avg_gems_per_hit = 15.0 # SKILL 獎勵平均值
        ...
        try:
            ...
            if resp.status_code == 200:
                ...
                if self.initial_gems == 0: 
                    ...
                    msg = f"🚀 機器人啟動成功 (重啟)\n💎 初始寶石：{gems}"
                    # self.send_telegram(msg) # 避免太吵可關閉
                return True
