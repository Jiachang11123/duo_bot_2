...
# 攻擊參數 (稍微調低以避免秒鎖)
DEFAULT_THREADS = 5   # 建議 5~10
DEFAULT_BATCH = 40    # 建議 30~50
DEFAULT_DELAY = 0.01
NOTIFY_INTERVAL = 1800 # 改為 30 分鐘報告一次 (安全模式)
...
class DuoGemNuclear:
    def __init__(self, token, reward_id):
        ...
        self.start_time = 0
        self.initial_gems = 0
        self.avg_gems_per_hit = 17.7 # 修正為 SKILL 的平均值
    ...
    async def fetch_user_data(self, session):
        try:
            ...
            if resp.status_code == 200:
                ...
                await self.suicide_restart() # 遇到阻擋直接自殺
                return False
        except:
            await self.suicide_restart() # 連線錯誤也自殺
        return False
    ...
    async def _send_patch(self, session, url, payload):
        try:
            ...
            elif resp.status_code in [403, 429]: await self.suicide_restart() # 遇到阻擋直接自殺
            else: self.stats['failed'] += 1
        except: self.stats['failed'] += 1 # 連線失敗不自殺，只計數，除非 fetch 也失敗
...
if __name__ == "__main__":
    token = DEFAULT_TOKEN
    # 檢查 Magic ID
    if "SKILL_COMPLETION" not in MAGIC_ID:
