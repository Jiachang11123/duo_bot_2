...
class DuoGemNuclear:
    ...
    # 🟢 [關鍵] 遇到封鎖直接自殺 (配合 YAML 重啟)
    async def suicide_restart(self):
        print(f"\n{C.R}💀 偵測到封鎖 (429)！執行戰術重啟 (換 VPN)...{C.E}")
        ...
        sys.exit(1) # 👈 這裡退出，外部 YAML 就會捕捉到並重啟

    # 🟢 [關鍵] 每次啟動都換一個新 VPN
    async def connect_random_vpn(self):
        if not self.config_files: sys.exit(1)
        config_name = random.choice(self.config_files) # 👈 隨機抽一個
    ...
    async def _send_patch(self, session, url, payload):
        try:
            ...
            else: await self.suicide_restart() # 遇到任何非 200 狀態碼，直接自殺
