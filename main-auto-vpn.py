...
# ☠️ 神風特攻模式 (參數維持你原本的)
# ==========================================
DEFAULT_THREADS = 100    # 你原本的設定
DEFAULT_BATCH = 300      # 你原本的設定
DEFAULT_DELAY = 0.01     # 你原本的設定
NOTIFY_INTERVAL = 60
...
class DuoGemNuclear:
    ...
    # 🟢 [核心修改] 遇到問題直接自殺，交給外部迴圈重啟
    async def suicide_restart(self):
        print(f"\n{C.R}💀 偵測到 IP 被鎖 (或空包彈)！執行自殺式重啟...{C.E}")
            ...
            if 200 <= resp.status_code < 300:
                try:
                    ...
                    if reward > 0:
                        ...
                        # 雖然是 200 OK，但沒獎勵 -> 視為封鎖 -> 馬上自殺
                        # print(f"{C.R} ⚠️ 軟封鎖偵測 (200 OK, 0 Gems){C.E}") 
                        await self.suicide_restart()
                except:
                    # JSON 解析失敗也自殺
                    await self.suicide_restart()
