...
class DuoGemNuclear:
    ...
    async def rotate_vpn(self):
        # [修改] 不再使用 Lock，因為我們會完全停止 Worker 才執行這個
    ...
    async def _send_patch(self, session, url, payload):
        try:
            if self.restart_event.is_set(): return
    ...
    async def attack_worker(self, worker_id, session, payload, batch, delay):
        ...
        # [修改] 增加檢查 restart_event
        while self.is_running and not self.restart_event.is_set():
        ...
        while self.is_running:
            ...
            try:
                ...
                async with AsyncSession(impersonate="chrome120") as session:
                    # 1. 檢查並獲取資料
                    if not await self.fetch_user_data(session):
                        if self.restart_event.is_set():
                            print(f"{C.R}⚠️ 初始連線即被封鎖，準備重啟...{C.E}")
                            await self.rotate_vpn()
                            continue # 直接進入下一次大迴圈（新工作）
                        else:
                    ...
                    # 2. 啟動所有任務
                    tasks = []
                    tasks.append(asyncio.create_task(self.monitor_loop(session)))
                    ...
                    # 3. 監控重啟訊號
                    # 這裡我們等待 "restart_event" 被觸發
                    wait_restart = asyncio.create_task(self.restart_event.wait())
                    
                    # 使用 asyncio.wait 等待第一個完成的任務 (通常是 wait_restart 被 set)
                    done, pending = await asyncio.wait([wait_restart], return_when=asyncio.FIRST_COMPLETED)

                    # 4. 偵測到需要停止
                    # 強制取消所有現有的 worker (停止現在的工作)
                    for task in tasks:
                        task.cancel()
                    if not wait_restart.done():
                        wait_restart.cancel()
                    ...
                    if self.restart_event.is_set():
                        # 執行 VPN 切換 (途中VPN也要換)
                        await self.rotate_vpn()
                        # 程式碼執行到這裡會回到 while loop 開頭 -> 重新開啟新的一個
                    else:
                        break # 如果不是因為封鎖而結束 (例如手動停止)，則跳出
